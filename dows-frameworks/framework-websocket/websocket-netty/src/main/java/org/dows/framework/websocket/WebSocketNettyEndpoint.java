...
import org.dows.framework.websocket.path.AntPathMatcherWrapper;
import org.dows.framework.websocket.path.DefaultPathMatcher;
import org.dows.framework.websocket.resolver.PathVariableMapMethodArgumentResolver;
import org.dows.framework.websocket.resolver.PathVariableMethodArgumentResolver;
 ...
 */
public class WebSocketNettyEndpoint implements WebSocketServerEndpoint {
    ...
    private final Set<WebSocketPathMatcher> pathMatchers = new HashSet<>();

    private static final InternalLogger log = InternalLoggerFactory.getInstance(WebSocketNettyEndpoint.class);

    public WebSocketNettyEndpoint(WebSocketMethodMapping methodMapping, WebSocketEndpointConfig config, String path) {
