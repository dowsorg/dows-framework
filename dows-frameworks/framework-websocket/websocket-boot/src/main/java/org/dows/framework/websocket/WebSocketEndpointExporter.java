...
public class WebSocketEndpointExporter extends ApplicationObjectSupport implements SmartInitializingSingleton, BeanFactoryAware {
    ...
    private List<MethodArgumentResolver> resolvers;

    public WebSocketEndpointExporter(ResourceLoader resourceLoader,List<MethodArgumentResolver> resolvers) {
    ...
    private void init() {
        for (Map.Entry<InetSocketAddress, WebSocketServer> entry : addressWebsocketServerMap.entrySet()) {
            ...
            try {
                ...
                WebSocketServerEndpoint webSocketEndpointServer = websocketServer.getEndpoint();
    ...
    private void registerEndpoint(Class<?> endpointClass) {
        ...
        try {
            webSocketMethodMapping = new WebSocketMethodMapping(endpointClass, context,resolvers);
        ...
        if (websocketServer == null) {
            // Add Server
            WebSocketNettyEndpoint webSocketEndpointServer = new WebSocketNettyEndpoint(webSocketMethodMapping, webSocketEndpointConfig, path);
            websocketServer = new WebsocketNettyServer(webSocketEndpointServer, webSocketEndpointConfig);
            addressWebsocketServerMap.put(inetSocketAddress, websocketServer);
        } else {
            // Add path to the same port
            websocketServer.getEndpoint().addPathMethodMapping(path, webSocketMethodMapping);
        }
    }
