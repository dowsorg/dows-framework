package org.dows.framework.crud.api.utils;
...
public class EntityUtil {
    ...
    public static <S> List<? extends S> collectList(Collection<S> source, Function<? super S, ? extends S> action) {
        return (List) collectList(source, action, Collectors.toList());
    }
    ...
    public static <T, R> Object[] toArray(T[] source, Function<? super T, ? extends R> action) {
        Objects.requireNonNull(action);
        return Objects.nonNull(source) ? Arrays.stream(source).map(action).toArray() : (new ArrayList()).toArray();
    }
